...
class App:
    ...
    def __init__(self, master):
        ...
        self.stepper_1_stepnumber = tk.Entry(frame_stepper_1)
        self.stepper_steplabel = tk.Label(frame_stepper_1,text="Number of Steps:")
        self.stepper_stepinfo = tk.Label(frame_stepper_1,text="200 steps = 360deg")
        self.stepper_steplabel.grid(row=0,column=0)
        self.stepper_1_stepnumber.grid(row=0,column=1)
        self.stepper_stepinfo.grid(row=1,column=1)

        self.stepper_1_space = tk.Label(frame_stepper_1, text=" ")
        self.stepper_1_forward = tk.Button(frame_stepper_1, text="Stepper 1 CW", command=partial(self.move_stepper_single,'1','f'))
        self.stepper_1_reverse = tk.Button(frame_stepper_1, text="Stepper 1 CCW", command=partial(self.move_stepper_single,'1','b'))
        self.stepper_1_space.grid(row=3,column=1)
        self.stepper_1_forward.grid(row=4,column=1)
        self.stepper_1_reverse.grid(row=5,column=1)
    ...
    def move_stepper_single(self,steppernumber,direction): #stepper 1-4,a; direction: f,r
        numberofsteps = self.stepper_1_stepnumber.get()
        if numberofsteps == '':
            numberofsteps = '1'
        steppernumberprint = 'Stepper ' + steppernumber + ' moves ' + numberofsteps + ' steps'
        if steppernumber =='a':
            steppernumberprint = 'All steppers'
        if direction =='f':
            directionprint = 'clockwise'
        elif direction =='b':
            directionprint = 'counterclockwise'
        else:
